...
export class BookingsController {
  ...
  @UseGuards(AuthGuard("jwt"))
  @ApiOperation({ summary: "Request a booking (member)" })
  async createBooking(@Request() req: any, @Body() body: { slotId: string; notes?: string }) {
    const booking = await this.bookingsService.createBooking(req.user._id.toString(), body);
    return booking;
  }
  ...
  async getBookings(
    ...
    const bookings = await this.bookingsService.getBookings({ status, limit: limit ? Number(limit) : undefined });
    // Always return { bookings, total } so the frontend can destructure consistently
